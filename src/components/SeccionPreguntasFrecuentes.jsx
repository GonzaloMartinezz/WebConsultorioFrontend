                ...
                <div className={`px-8 overflow-hidden transition-all duration-500 ease-in-out ${openFaq === index ? 'max-h-[500px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="mt-6 flex flex-wrap gap-4">
                    {faq.q.includes("turno") && (
                      <Link to="/turnos" className="px-6 py-3 bg-accent-orange text-white rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-lg shadow-accent-orange/20">
                        Solicitar Turno Online
                      </Link>
                    )}
                    {faq.q.includes("App") && (
                      <>
                        <Link to="/login" className="px-6 py-3 bg-white text-primary rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-lg">
                          Iniciar Sesión
                        </Link>
                        <Link to="/login" className="px-6 py-3 bg-white/10 border border-white/20 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-white/20 transition-all">
                          Registrarse
                        </Link>
                      </>
                    )}
                    {faq.q.includes("obras sociales") && (
                      <a href="https://wa.me/5493816242482" target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-green-500 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-lg shadow-green-500/20">
