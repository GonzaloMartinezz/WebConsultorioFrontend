        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-3" ref={userMenuRef}>
            {user ? (
              <div className="relative">
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-64 rounded-xl border border-secondary bg-white p-4 shadow-xl">
                    ...
                    <p className="mt-1 font-bold text-text text-lg">
                    ...
                    <Link
                      to="/turnos"
                      className="block w-full rounded-lg bg-primary px-4 py-3 text-center text-sm font-semibold text-white hover:bg-primary/90 transition-colors"
              ...
              <Link
                to="/login"
                className="flex h-10 sm:h-10 items-center justify-center rounded-lg border-2 border-primary bg-transparent px-7 font-semibold text-primary hover:bg-primary hover:text-white transition-all"
              >
                Iniciar sesión
              </Link>
            )}
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-secondary bg-background shadow-lg">
          <div className="flex flex-col px-4 py-4 space-y-2">
            ...
            <div className="h-px w-full bg-secondary my-2 md:hidden block"></div>

            {/* Acciones en Móvil */}
            {user ? (
              <button
                onClick={handleLogout}
                className="block w-full text-left rounded-lg px-4 py-3 text-lg font-medium text-red-600 hover:bg-red-50"
              >
              ...
              <Link
                to="/login"
                className="block rounded-lg px-4 py-3 text-lg font-medium text-primary hover:bg-secondary"
                onClick={() => setMenuOpen(false)}
              >
                Iniciar sesión
              </Link>
            ...
            <Link
              to="/turnos"
              className="block rounded-lg bg-accent-orange px-4 py-4 text-center text-lg font-bold text-text shadow-sm"
              onClick={() => setMenuOpen(false)}
            >
              Reservar turno
            </Link>
